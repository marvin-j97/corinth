...
export default defineWorkflow(async () => {
  ...
  await sleep(3500);
  spawnCorinth(undefined, 4);

  persistenceTeardown();

  const queueName = "compact_interval";
  ...
  assert(existsSync(".corinth/queues/compact_interval/meta.json"));
  assert(!existsSync(".corinth/queues/compact_interval/items.jsonl"));
  ...
  return {
    ...
    steps: [
      ...(() => {
        ...
        for (let i = 0; i < NUM_ITEMS; i++) {
          steps.push({
            ...
            validate: () => {
              assert(
                existsSync(".corinth/queues/compact_interval/items.jsonl")
              );
            },
          });
        }
      ...
      },
      {
        ...
        validate: async () => {
          expect(
            JSON.parse(
              readFileSync(
                ".corinth/queues/compact_interval/meta.json",
                "utf-8"
              )
            ).num_acknowledged
          ).to.equal(2);
          ...
          const itemsInFile = readFileSync(
            ".corinth/queues/compact_interval/items.jsonl",
            "utf-8"
          )
            .split("\n")
            .filter(Boolean)
      ...
      {
        title: "Queue should be compacted",
        status: 200,
        url: queueUrl,
        resBody: yxc.object({
          message: yxc.string().equals("Queue info retrieved successfully"),
